...
public class TestVerifyDayFutureBooking
{
   WebDriver driver;
   String data = null;
   String city = null;
   ...
   public TestVerifyDayFutureBooking(String data)
   {
	 this.data = data;
   }
   ...
   @Test()
   public void testVerifyDayFutureBooking()
   {
	  ...
	  /** Getting drop locality for the final time even after the drop locality changes */
	  ...
	  new Assertion().assertString(customerNameFromAnsCal.toLowerCase(), customerNameFromConfPage.toLowerCase(), "The customer name in answer call page " + "'" + customerNameFromAnsCal + "'"
   ...
   public void cleanUp(ITestResult result)
   {
	  if (!result.isSuccess())
	  {
		 ...
		 Reporter.log("<a href = file:///" + new File(destDir + "/" + destFile).getAbsolutePath() + " target='_blank'" + "> "
			   + "<p align=\"left\", style= \"color: red; font: italic bold 12px/30px Georgia,serif;\" >Click here for screen shot of " + city + " " + bookingType + " " +cabType+"</p> </a>");
		 System.out.println("Taken screen shots");
	  }else
	  {
		  System.setProperty("org.uncommons.reportng.escape-output", "false");
		  Reporter.setCurrentTestResult(result);
		  Reporter.log("Test passed for advanced booking day for the combination " + city + "_" + cabTypeValue + "_" + bookingType);
	  }
	  new MyDriver().quitBrowser(driver);
   }

   @DataProvider(name = "cityName", parallel = true)
   public Object[][] getCityNames()
   {
	  driver = new MyDriver().getDriver();
	  Object obj[][] = new BookingTypes().getAllBookingTypesForCity(driver, "Rajkot", "Surat");
